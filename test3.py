...
def resnet101(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(torchvision.models.resnet.Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        model.load_state_dict(torchvision.models.resnet.model_zoo.load_url(torchvision.models.resnet.model_urls['resnet101']))
    return model
...
class ROIPooling(torch.nn.Module):
    ...
    def __init__(self,_size,_stride,has_backward=False):
        """
        input:
    ...
    def forward(self,input,rois):
        ...
        for i in range(num_rois):
            ...
            # modified, avoid the none tensor
            # roi[2]=min(h-1,roi[2])
            # roi[1]=min(w-1,roi[1])
